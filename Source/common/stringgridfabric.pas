...
uses
  Classes, SysUtils, Forms, Controls, Graphics, Grids, ExtCtrls;
...
Type
 ...
 PsgfCheckBoxCellPosition = ^TsgfCheckBoxCellPosition;
 TsgfCheckBoxCellPosition = record
   Col: Integer;
   Row: Integer;
 ...
 PsgfItemObjectData = ^TsgfItemObjectData;
 TsgfItemObjectData = record
 ...
 AllItemsObjectData = array of TsgfItemObjectData;
 ...
 TStringGridFab = class(TObject)
   private
     ...
     FDupText : Boolean;
     CheckedParentCells: array of TsgfCheckBoxCellPosition;
     CheckedChildCells: array of TsgfCheckBoxCellPosition;
     ChildCellsOfSelection: array of TsgfCheckBoxCellPosition;
     ClickedCell: array of TsgfCheckBoxCellPosition;
   ...
   public
     ...
     function AutoFillStringGridCol(aStringGrid: TStringGrid; aCol: Integer): Integer;
...
begin
  ...
  if PsgfItemObjectData(_stringGrid.Objects[lRec.acol, lRec.aRow]) <> nil then begin
    pItem := PsgfItemObjectData(_stringGrid.Objects[lRec.aCol, lRec.aRow]);
      ...
      for i := 0 to Length(lrec.sgarray)-1 do begin
        for r := 0 to TStringGrid(lrec.sgarray[i]).RowCount-1 do begin
          for c := 0 to TStringGrid(lrec.sgarray[i]).ColCount-1 do begin
            pItem := PsgfItemObjectData(TStringGrid(lrec.sgarray[i]).Objects[c,r]);
...
begin
  ...
  // parent is checked
  if (PsgfCheckBoxCellPosition(cbToggled)^.Col = 1) and (PsgfCheckBoxCellPosition(cbToggled)^.CbIsChecked) then begin
    if CheckedParentCells = Nil Then
      Setlength(CheckedParentCells, 0);

    if not DoesParentGuidExist(PsgfCheckBoxCellPosition(cbToggled)^.aGuid) then begin
      Setlength(CheckedParentCells, Length(CheckedParentCells)+1);
      CheckedParentCells[Length(CheckedParentCells)-1].aGuid := PsgfCheckBoxCellPosition(cbToggled)^.aGuid;
      CheckedParentCells[Length(CheckedParentCells)-1].aGridPtr := PsgfCheckBoxCellPosition(cbToggled)^.aGridPtr;
      CheckedParentCells[Length(CheckedParentCells)-1].sgName := TStringgrid(PsgfCheckBoxCellPosition(cbToggled)^.aGridPtr).Name; // makkelijker leesbaar als gecontroleerd wordt in wlke grid de parent zit dan de pointer
    end;
  end
  // uncheck a parent
  else if (PsgfCheckBoxCellPosition(cbToggled)^.Col = 1) and (not PsgfCheckBoxCellPosition(cbToggled)^.CbIsChecked) then begin
    if DoesParentGuidExist(PsgfCheckBoxCellPosition(cbToggled)^.aGuid) then begin
      for i:= Length(CheckedParentCells)-1 downto 0 do begin
        if CheckedParentCells[i].aGuid = PsgfCheckBoxCellPosition(cbToggled)^.aGuid then begin
...
var
  PGuid : String;
  i, j, c, r: Integer;
  Pobj :  PsgfItemObjectData;
begin
  if (PsgfCheckBoxCellPosition(cbToggled)^.Col = 1) and (not PsgfCheckBoxCellPosition(cbToggled)^.CbIsChecked) then begin
    pGuid := PsgfCheckBoxCellPosition(cbToggled)^.aGuid;
    ...
    for i:=0 to length(AllStringGrids)-1 do begin
      for r:=0 to AllStringGrids[i].RowCount -1 do begin
        for c:=0 to AllStringGrids[i].ColCount -1 do begin
          if PsgfItemObjectData(TstringGrid(AllStringGrids[i]).Objects[c,r]) <> nil then begin
            Pobj :=  PsgfItemObjectData(TstringGrid(AllStringGrids[i]).Objects[c,r]);
...
begin
  ...
  if PsgfCheckBoxCellPosition(cbToggled)^.CbIsChecked then begin
          ...
          if AllStringGrids[j] = _sg then begin
            for r:=0 to _sg.RowCount-1 do begin
              for c:=0 to _sg.ColCount -1 do begin
                if PsgfItemObjectData(_sg.Objects[c,r]) <> nil then begin
                  pItem := PsgfItemObjectData(_sg.Objects[c,r]);

                  if CheckedChildCells[i].aGuid = PsgfItemObjectData(_sg.Objects[c,r])^.Guid then begin
...
var
  i, j: Integer;
  pCbPos : PsgfCheckBoxCellPosition;
begin
  pCbPos := PsgfCheckBoxCellPosition(cbToggled);

  if (PsgfCheckBoxCellPosition(cbToggled)^.Col = 2) and (PsgfCheckBoxCellPosition(cbToggled)^.CbIsChecked) then begin
    if CheckedChildCells = Nil Then
      Setlength(CheckedChildCells, 0);

    // add child to the array
    if not DoesChildGuidExist(PsgfCheckBoxCellPosition(cbToggled)^.aGuid) then begin
      for i:= 0 to length(CheckedParentCells)-1 do begin
        Setlength(CheckedChildCells, Length(CheckedChildCells)+1);
        CheckedChildCells[Length(CheckedChildCells)-1].aGuid := PsgfCheckBoxCellPosition(cbToggled)^.aGuid;
        ...
        // add to sgObject
        setlength(PsgfItemObjectData(pCbPos^.RowObject)^.Parent_guid , Length(PsgfItemObjectData(pCbPos^.RowObject)^.Parent_guid)+1);
        PsgfItemObjectData(pCbPos^.RowObject)^.Parent_guid[Length(PsgfItemObjectData(pCbPos^.RowObject)^.Parent_guid)-1] := CheckedParentCells[i].aGuid;
        PsgfItemObjectData(pCbPos^.RowObject)^.Action := caInsert;
  ...
  else if (PsgfCheckBoxCellPosition(cbToggled)^.Col = 2) and (not PsgfCheckBoxCellPosition(cbToggled)^.CbIsChecked) then begin
    if DoesChildGuidExist(PsgfCheckBoxCellPosition(cbToggled)^.aGuid) then begin

      for i:= Length(CheckedChildCells)-1 downto 0 do begin
        if CheckedChildCells[i].aGuid = PsgfCheckBoxCellPosition(cbToggled)^.aGuid then begin
          if CheckedChildCells[i].Action = caInsert then begin
            ...
            pCbPos := PsgfCheckBoxCellPosition(cbToggled);
             ...
             setlength(PsgfItemObjectData(pCbPos^.RowObject)^.Parent_guid , Length(PsgfItemObjectData(pCbPos^.RowObject)^.Parent_guid)+1);
            for j:= length(PsgfItemObjectData(pCbPos^.RowObject)^.Parent_guid)-1 downto 0 do begin
              if PsgfItemObjectData(pCbPos^.RowObject)^.Parent_guid[j] = CheckedChildCells[i].Parent_guid[0] then begin
                Delete(PsgfItemObjectData(pCbPos^.RowObject)^.Parent_guid,j,1);
              end;
            end;

            PsgfItemObjectData(pCbPos^.RowObject)^.Action := 'Read';
...
procedure TStringGridFab.SetStringGridWidth(anObj: TObject);
var
  _sg: TStringGrid;
  _pnl: TPanel;
  _sb: TScrollBox;
...
begin
  ...
  if anObj is TStringGrid then begin
    _sg:= TStringGrid(anObj);
    i:= StrToInt(ExtractNumberFromString(_sg.Name));

    if _sg.Parent is TPanel then begin
      _pnl:= TPanel(_sg.Parent); // data panel

      if i=1 then begin  // Is most Left Stringgrid
        if _pnl.Parent is TPanel then begin // Body panel
          _pnl:= TPanel(_pnl.Parent);
          if _pnl.Parent is TScrollBox then begin
            _sb:= TScrollBox(_pnl.Parent);

            if TPanel(_sb.Parent) is TPanel then begin
              _pnl:= TPanel(_sb.Parent);
              _pnl.Width:= AutoFillStringGridCol(_sg, 3) + 50 + 100;
            end;
          end;
        end;
      end
      else begin // All other StringGrids
        TPanel(_pnl.Parent).Width:= AutoFillStringGridCol(_sg, 3) + 50 + 100;
      end;
    end;
  end;
end;

function TStringGridFab.AutoFillStringGridCol(aStringGrid: TStringGrid;
...
const
  ...
  MinWidth = 100;
begin
  Longest := aStringGrid.Canvas.TextWidth(aStringGrid.Cells[aCol, 0]);
  for i := 0 to aStringGrid.RowCount - 1 do
  begin
    Temp := aStringGrid.Canvas.TextWidth(aStringGrid.Cells[aCol, i]);
    if Temp > Longest then
      Longest := Temp;
  end;
  //aStringGrid.ColWidths[aCol] := Longest + Padding;
  if Longest + Padding > MinWidth then begin
    result := Longest + Padding;
  end
  else begin
    result := MinWidth;
  end;
...
begin
  tmpCounter := 1;
  PGuid := PsgfCheckBoxCellPosition(cbToggled)^.aGuid;  // the unchecked cell

  { #todo : Kan versnelt worden door level te gaan gebruiken. Nu wordt van elke stringgrid elke rij en kolom bekeken }
  for i:=0 to length(AllStringGrids)-1 do begin
    for r:=0 to AllStringGrids[i].RowCount-1 do begin
      for c:=0 to AllStringGrids[i].ColCount -1 do begin
        pItem := PsgfItemObjectData(AllStringGrids[i].Objects[c,r]);
        if pItem <> nil then begin
...
function TStringGridFab.ExtractNumberFromString(const Value: string): String;
var
  ch: char;
  Index, Count: integer;
begin
...
begin
  ...
  if PsgfCheckBoxCellPosition(cbToggled)^.CbIsChecked Then begin
    RetrieveChilds(PsgfCheckBoxCellPosition(cbToggled)^.aGuid);
  end;

  CheckChild(cbToggled);
  if not PsgfCheckBoxCellPosition(cbToggled)^.CbIsChecked Then begin
