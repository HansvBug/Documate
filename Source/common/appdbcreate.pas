...
begin
  if dbFile = '' then begin
    result := false;
    AddDbCreateMessage(mError, 'Geen database aangemaakt. Bestandnaam is leeg.');
  end
  else begin
    if FileExists(dbFile) then begin
      RenameFile(dbFile, StringReplace(dbFile, '.db', '_Expired.db', [rfIgnoreCase]));
      AddDbCreateMessage(mInformation, 'The file : ' + dbFile + ' is renamed.');
    ...
    AddDbCreateMessage(mInformation, 'Aanmaken nieuw leeg database bestand op de locatie: '+ dbFile);
    ...
    try
      ...
      AddDbCreateMessage(mInformation, 'Leeg database bestand is aangemaakt.');
      FNewDatabaseCreated := True;
      result := true;
    except
      on E : Exception do begin
          AddDbCreateMessage(mError, 'Fout bij het aanmaken van een leeg database bestand.');
          AddDbCreateMessage(mError, 'Melding:');
          AddDbCreateMessage(mError, E.Message);
          AddDbCreateMessage(mError, '');
        FError := true;
        result := false;
      end;
    end;
...
begin
  try
    ...
    FSQLite3Connection.Close();
    AddDbCreateMessage(mInformation, 'Database instelling: auto_vacuum = incremental.');
  except
    on E : Exception do begin
      AddDbCreateMessage(mError, 'Fout bij maken van een database instelling. (auto_vacuum).');
      AddDbCreateMessage(mError, 'Melding:');
      AddDbCreateMessage(mError, E.Message);
      AddDbCreateMessage(mError, '');
      FError := true;
...
begin
  try
    ...
    FSQLite3Connection.ExecuteDirect('Begin Transaction');     //Start a transaction for SQLdb to use
    FSQLite3Connection.Close();
    AddDbCreateMessage(mInformation, 'Database instelling: journal_mode = WAL.');
  except
    on E : Exception do begin
      AddDbCreateMessage(mError, 'Fout bij maken van een database instelling. (journal_mode).');
      AddDbCreateMessage(mError, 'Melding:');
      AddDbCreateMessage(mError, E.Message);
      AddDbCreateMessage(mError, '');
      FError := true;
...
begin
  try
    ...
    FSQLite3Connection.ExecuteDirect('Begin Transaction');     //Start a transaction for SQLdb to use
    FSQLite3Connection.Close();
    AddDbCreateMessage(mInformation, 'Database instelling: synchronous = normal.');
  except
    on E : Exception do begin
      AddDbCreateMessage(mError, 'Fout bij maken van een database instelling. (synchronous).');
      AddDbCreateMessage(mError, 'Melding:');
      AddDbCreateMessage(mError, E.Message);
      AddDbCreateMessage(mError, '');
      FError := true;
...
begin
  try
    ...
    FSQLite3Connection.ExecuteDirect('Begin Transaction');     //Start a transaction for SQLdb to use
    FSQLite3Connection.Close();
    AddDbCreateMessage(mInformation, 'Database instelling: temp_store = memory.');
  except
    on E : Exception do begin
      AddDbCreateMessage(mError, 'Fout bij maken van een database instelling. (temp_store).');
      AddDbCreateMessage(mError, 'Melding:');
      AddDbCreateMessage(mError, E.Message);
      AddDbCreateMessage(mError, '');
      FError := true;
...
begin
  try
    ...
    FSQLite3Connection.ExecuteDirect('Begin Transaction');     //Start a transaction for SQLdb to use
    FSQLite3Connection.Close();
    AddDbCreateMessage(mInformation, 'Database instelling: USER_VERSION = ' + version);
  except
    on E : Exception do begin
      AddDbCreateMessage(mError, 'Fout bij maken van een database instelling. (USER_VERSION).');
      AddDbCreateMessage(mError, 'Melding:');
      AddDbCreateMessage(mError, E.Message);
      AddDbCreateMessage(mError, '');
      FError := true;
...
begin
  try
    AddDbCreateMessage(mInformation, 'Start aanmaken tabel: ' + TableName + '. (Versie: ' + Version + ').');
    FSQLite3Connection.Close();
    FSQLite3Connection.DatabaseName := dbFile;
    FSQLite3Connection.Open;
    ...
    on E : Exception do begin
      AddDbCreateMessage(mError, 'Fout bij het aanmaken van de tabel: ' + TableName + '. (Versie: ' + Version + ').');
      AddDbCreateMessage(mError, 'Melding:');
      AddDbCreateMessage(mError, E.Message);
      AddDbCreateMessage(mError, '');
      FError := true;
...
begin
  ...
  try
    ...
    FSQLTransaction.Commit;
    FSQLite3Connection.Close();
    AddDbCreateMessage(mInformation, 'Toegevoegd aan de tabel '+ SETTINGS_META + ' is : ' + aKey + ' - ' + Avalue + '.');
  except
    on E: EDatabaseError do begin
      AddDbCreateMessage(mError, 'Het invoeren van "Versienummmer" is mislukt.');
      AddDbCreateMessage(mError, 'Melding:');
      AddDbCreateMessage(mError, E.Message);
      AddDbCreateMessage(mError, '');
      FError := true;
...
begin
  SqlString := 'select VALUE from ' + SETTINGS_META + ' where KEY = :KEY';
    try
      ...
      AddDbCreateMessage(mInformation, 'Versie is opgevraagd. (Tabel: '+ SETTINGS_META + ').');
      Result := Version;
  except
    on E: EDatabaseError do begin
      AddDbCreateMessage(mError, 'Opvragen versienummer is mislukt.');
      AddDbCreateMessage(mError, 'Melding:');
      AddDbCreateMessage(mError, E.Message);
      AddDbCreateMessage(mError, '');
      FError := true;
...
begin
  ...
  try
    ...
    FSQLTransaction.Commit;
    FSQLite3Connection.Close();
    AddDbCreateMessage(mInformation, 'Versie is bijgewerkt. (Tabel: '+ SETTINGS_META + ', Versie: ' + Version + ').');
  except
    on E: EDatabaseError do begin
      AddDbCreateMessage(mError, 'Het actualiseren van '+ SETTINGS_META + ' is mislukt.');
      AddDbCreateMessage(mError, 'Melding:');
      AddDbCreateMessage(mError, E.Message);
      AddDbCreateMessage(mError, '');
      FError := true;
...
begin
  if FileExists(dbFile) then begin
    ...
    if not FError then begin
      AddDbCreateMessage(mInformation, 'Het aanmaken/bijwerken van de database (tabellen) is gereed. (Versie: ' + Version + ').');
      AddDbCreateMessage(mInformation, '');
    end;
  end
  else begin  // database file does not exists
    AddDbCreateMessage(mError, 'De database is niet gevonden.');
    FError := true;
  end;
end;
