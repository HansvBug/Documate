...
type
  ...
  PappdbGetColumnNameRec = ^TappdbGetColumnNameRec;
  TappdbGetColumnNameRec = record
    ColumnName : string;
    Key        : String;
    Value      : String;
    AllColNames: array of TappdbGetColumnNameRec;
    aGrid : TObject;
    Success : Boolean;
  end;
  appdbAllColumnNames = array of TappdbGetColumnNameRec;
  ...
  TAppDbMaintainItems = class(TAppDatabase)
    private
      ...
      function GetColNames: appdbAllColumnNames;
    ...
    public
      ...
      // Config view
      function GetAllColumnNames : appdbAllColumnNames;
      function SaveColumnNames(sgData: Pointer): TappdbGetColumnNameRec;
...
begin
  ...
  try
    ...
    on E : Exception do begin
      Result := False;
      AddDbMaintainMessage(mError, 'Fout bij het ophalen van alle items.');
      AddDbMaintainMessage(mError, 'Melding:');
      AddDbMaintainMessage(mError, E.Message);
      AddDbMaintainMessage(mError, '');
    end;
  end;
end;
...
begin
  ...
  try
    ...
    on E : Exception do begin
      AddDbMaintainMessage(mError, 'Fout bij het ophalen van de relaties.');
      AddDbMaintainMessage(mError, 'Melding:');
      AddDbMaintainMessage(mError, E.Message);
      AddDbMaintainMessage(mError, '');
    end;
    ...
    if AllItems[i].Guid <> '' then begin
      if AllItems[i].Action = iaCreate then begin
        ...
        try
          ...
          AddDbMaintainMessage(mInformation, 'Item ' + AllItems[i].Name + ' is toegevoegd aan tabel ' + ITEMS + '.');
        except
          on E: EDatabaseError do
            begin
              AddDbMaintainMessage(mError, 'Het invoeren van een nieuw item in de tabel LEVEL_' + IntToStr(AllItems[i].Level) +' is mislukt.');
              AddDbMaintainMessage(mError, 'Melding:');
              AddDbMaintainMessage(mError, E.Message);
              CanContinue := False;
            end;
      ...
      AddDbMaintainMessage(mError, 'Nieuw item niet opgeslagen. Guid ontbreekt.');
    end
  end;
  result := CanContinue;
end;
    ...
    if AllItems[i].Guid <> '' then begin
      if AllItems[i].Action = iaUpdate then begin
        ...
        try
          ...
          AddDbMaintainMessage(mInformation, 'Item ' + AllItems[i].Name + ' is gemuteerd in tabel ' + ITEMS + '.');
        except
          on E: EDatabaseError do
            begin
              AddDbMaintainMessage(mError, 'Het hernoemen van een bestaand item is mislukt.');
              AddDbMaintainMessage(mError, 'Melding:');
              AddDbMaintainMessage(mError, E.Message);
              CanContinue := False;
            end;
    ...
    if AllItems[i].Guid <> '' then begin
      if AllItems[i].Action = iaDelete then begin
        ...
        try
          ...
          AllItems[i].Action := iaRead; // Update is done.
          AddDbMaintainMessage(mInformation, 'Item ' + AllItems[i].Name + ' is verwijdert uit de tabel ' + ITEMS + '.');

        except
          on E : Exception do begin
            AddDbMaintainMessage(mError, 'Fout bij het verwijderen van een item.');
            AddDbMaintainMessage(mError, 'Melding:');
            AddDbMaintainMessage(mError, E.Message);
...
begin
  ...
  if (aGuid <> '') and (PGuid <> '') then begin
    ...
    try
      ...
      AddDbMaintainMessage(mInformation, 'Relatie toegevoegd. Parent: ' + PGuid + '; child: ' + aGuid + '.');
    except
      on E : Exception do begin
        AddDbMaintainMessage(mError, 'Fout bij het toevoegen van een realatie.');
        AddDbMaintainMessage(mError, 'Melding:');
        AddDbMaintainMessage(mError, E.Message);
      end;
    end;
...
begin
  ...
  try
    ...
    CanContinue := true;
    AddDbMaintainMessage(mInformation, 'Relatie verwijderd. Parent: ' + PGuid + '; child: ' + aGuid + '.');
  except
    on E : Exception do begin
      AddDbMaintainMessage(mError, 'Fout bij het verwijderen van een realatie.');
      AddDbMaintainMessage(mError, 'Melding:');
      AddDbMaintainMessage(mError, E.Message);
    end;
  end;
...
function TAppDbMaintainItems.GetColNames: appdbAllColumnNames;
var
  SqlText : String;
  allNames : appdbAllColumnNames = nil;
  collData : TappdbGetColumnNameRec;
  i : Integer;
begin
  ...
  try
    ...
    on E : Exception do begin
      AddDbMaintainMessage(mError, 'Fout bij het ophalen van de kolom namen.');
      AddDbMaintainMessage(mError, 'Melding:');
      AddDbMaintainMessage(mError, E.Message);
      Result := nil;
    end;
...
begin
  ...
  try
    ...
    on E: EDatabaseError do
      begin
        AddDbMaintainMessage(mError, 'Het actualiseren van de kolomnamen is mislukt.');
        AddDbMaintainMessage(mError, 'Melding:');
        AddDbMaintainMessage(mError, E.Message);
        FCanSave := False;
      end;
...
function TAppDbMaintainItems.GetAllColumnNames: appdbAllColumnNames;
begin
  Result := GetColNames;
end;

function TAppDbMaintainItems.SaveColumnNames(sgData: Pointer): TappdbGetColumnNameRec;
var
  lRec : PappdbGetColumnNameRec;
  r : Integer;
  _sg : TStringGrid;
begin
  lRec := PappdbGetColumnNameRec(sgData);
